  (function(){
    ...
    function buyerLink(d){
      var u=new URL(d.cartUrl||d.url);
      if(d.cartUrl){if(u.origin!==location.origin||u.pathname!==location.pathname||!/^#\/live\?claim=/.test(u.hash))throw new Error('Unexpected buyer cart link');}
      else if(u.protocol!=='https:'||u.username||u.password||u.port||!/(^|\.)square\.(link|site)$/.test(u.hostname))throw new Error('Unexpected checkout link');
      $('#claimPaymentLink').href=u.href;$('#claimPaymentLink').textContent=d.cartUrl?'Open buyer’s cart ↗':'Open Square checkout ↗';$('#claimPaymentUrl').value=u.href;
    }
    function render(){
