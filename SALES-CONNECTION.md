...
- Set the exact webhook URL and signature secret; subscribe to `payment.updated` and the existing inventory/order events. Test the Worker privately in Admin, then set GitHub's public `TL_API_URL` repository variable and run Publish site. See [API connection guide](API-CONNECT.md).
...
The ordinary shop cart still uses the existing TCGplayer-snapshot/ad-hoc checkout path; it has **not** been converted into an exact condition-level Square catalog map. `SHOP_CHECKOUT_ENABLED=true` permits sandbox testing only; the route rejects production even with that switch. Adding Square keys does not activate it. Store-credit redemption, table-booking payments and automatic refunds are separate workflows, not enabled by this release.
