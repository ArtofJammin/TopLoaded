...
See [sales connection and acceptance](../SALES-CONNECTION.md). `GET /live/checkout/status` reports readiness; `GET /live/catalog?q=` (staff) searches Square variations. `POST /live/claims` may include `squareVariationId`, `tcgProductId` and `fulfillment`. Staff then call `POST /live/claims/:id/checkout` for the private payment link and a `cartUrl` buyer handoff, or `POST /live/claims/:id/cancel-checkout` to cancel an unpaid link. `CLAIM_CHECKOUT_ENABLED=true`, Square, signed webhooks and shared storage are required. Public claims exclude Square IDs and checkout URLs. Payment confirmation is automated only for these linked claims; Square performs their inventory adjustment, not a second manual decrement. The legacy ad-hoc `/checkout` route remains sandbox-only; the frontend never submits TCGplayer or stream groups to it.

`POST /live/claims/:id/payment` accepts `{access}` from the private buyer link (60 requests/minute per IP, 1 KB JSON limit). It returns only that claim's server-side card, price, status, fulfillment/shipping, mode and an allowlisted Square URL while unpaid and enabled. Missing, wrong, expired or different-claim tokens return 403. This endpoint does not create orders, mark paid, or adjust stock. Responses are `no-store`. The 256-bit random grant lasts seven days; KV stores only its SHA-256 hash. The URL token is a private bearer capability, not an account or staff session: anyone with the link can open that claim's checkout. Send it privately. The frontend removes the token from the address bar and keeps it only in API-bound session storage, never in the persistent cart or public claims feed. `SITE_URL` must be the canonical HTTPS frontend URL (HTTP localhost is allowed only in sandbox).
