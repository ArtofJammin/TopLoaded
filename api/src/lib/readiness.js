...
export function setupReport(env, cfg = {}) {
  ...
  add('site', 'Published site / browser access', { SITE_URL: httpsUrl(env.SITE_URL), SITE_ORIGIN: origins.length > 0 && origins.every(o => httpsUrl(o) && new URL(o).origin === o) && origins.includes(siteOrigin) }, 'Use the full Pages site URL including /TopLoaded/; CORS uses only the origin, without the repository path.');
